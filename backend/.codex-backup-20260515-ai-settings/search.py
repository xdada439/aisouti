...
def search_question(req: SearchRequest, db: Session = Depends(get_db)):
    ...
    # Step 1: 题库匹配
    logger.info("--- Step 1: Question Bank Match ---")
    ...
    if result:
        logger.info("Question bank MATCHED: answer=%s source=%s confidence=%.3f",
                     result["answer"], result["source"], result["confidence"])
        return {"success": True, **result}

    logger.info("Question bank: NO MATCH, falling back to RAG")

    # Step 2: 教材知识库RAG检索
    logger.info("--- Step 2: Knowledge Base RAG ---")
    ...
    logger.info("RAG result: answer='%s' source='%s' confidence=%.3f chunks=%d matchDetail='%s'",
